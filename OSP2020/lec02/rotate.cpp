...
//    float sq_col = ceil(col * sin(radian) + row * cos(radian));
    float sq_col = ceil(row * sin(radian) + col * cos(radian));
    float sq_row = ceil(col * sin(radian) + row * cos(radian));
        ...
        for (int j = 0; j < sq_col; j++) {
            ...
            if ((y >= 0) && (y <= (row - 1)) && (x >= 0) && (x <= (col - 1))) {
                if (!strcmp(opt, "nearest")) {
                    ...
                    output.at<Vec3b>(i, j) =
                        (input.at<Vec3b>(y1, x1) * (1 - lambda) + input.at<Vec3b>(y1, x2) * lambda) * (1 - mu) +
                        (input.at<Vec3b>(y2, x1) * (1 - lambda) + input.at<Vec3b>(y2, x2) * lambda) * mu;
                }
            }
        }
    }
