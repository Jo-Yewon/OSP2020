...
Mat Mirroring(const Mat input, int n)
{
    ...
    for (int i = n; i < row + n; i++) {
        for (int j = n; j < col + n; j++) {
            input2.at<G>(i, j) = input.at<G>(i - n, j - n);
        }
    }
    for (int i = n; i < row + n; i++) {
        for (int j = 0; j < n; j++) {
            input2.at<G>(i, j) = input2.at<G>(i, 2 * n - j);
        }
        for (int j = col + n; j < col2; j++) {
            input2.at<G>(i, j) = input2.at<G>(i, 2 * col - 2 + 2 * n - j);
        }
    }
    for (int j = 0; j < col2; j++) {
        for (int i = 0; i < n; i++) {
            input2.at<G>(i, j) = input2.at<G>(2 * n - i, j);
        }
        for (int i = row + n; i < row2; i++) {
            input2.at<G>(i, j) = input2.at<G>(2 * row - 2 + 2 * n - i, j);
        }
    }
