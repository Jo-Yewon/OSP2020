...
int nearestNeighborSecond(Mat& vec, vector<KeyPoint>& keypoints, Mat& descriptors, int firstIndex){
    int neighbor = -1;
    double minDist = 1e6;
...
void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1,
    ...
    for (int i = 0; i < descriptors1.rows; i++) {
        ...
        if (ratio_threshold) {
            ...
            double k1 = euclidDistance(desc1, v1);
            
            int nn2 = nearestNeighborSecond(desc1, keypoints2, descriptors2, nn);
            if (nn2 == -1) continue;
            Mat v2 =descriptors2.row(nn2);
            double k2 = euclidDistance(desc1, v2);
            
            if (k1 / k2 >= 0.4) continue;
        }

        // Refine matching points using cross-checking
        if (crossCheck) {
            Mat desc2 = descriptors2.row(nn);
            int nnCross = nearestNeighbor(desc2, keypoints1, descriptors1);
            if (nnCross != i) continue;
        }
